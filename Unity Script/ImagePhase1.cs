...
public class ImagePhase1 : MonoBehaviour
{
    ...
    private string apiKey = "_YOUR_API_KEY";
    ...
    private async Task<string> GetGPTResponse(string combinedInput)
    {
        string contentToSend = "I process book images using Tesseract for OCR text and the BLIP model for image-text retrieval. I'll provide outputs from both models and ask queries based on them. Please use the outputs to answer my questions.";
        string fullMessage = $"{contentToSend}\n\n{combinedInput}";

        var messages = new List<ChatMessage>
    {
        new ChatMessage { Role = "system", Content = "You are an assistant providing concise responses to user queries." },
        new ChatMessage { Role = "user", Content = fullMessage }
    };
    ...
    private IEnumerator SendToGPT()
    {
        ...
        Task<string> gptResponseTask = GetGPTResponse(combinedInput);
